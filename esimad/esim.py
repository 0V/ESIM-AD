...
def residue(
    X_diff: NDArray[np.double],
    Y: NDArray[np.double],
    alph: float,
    beta: NDArray[np.double],
    bb: Union[float, NDArray[np.double]],
) -> float:
    ww = wgt(X_diff, bb)
...
def solve_wlr_simple(
    x: NDArray[np.double], Y: NDArray[np.double], i_ctr: int, bb=None, x_var=None, Y_var=None, spp=None
):
    """
    X: [n, D]
    Y: [n, 1]
...
def solve_wlr_varopt(
    ...
    lmb1 = np.sum((bias_list - bias_mean) * (h2_list - h2_mean)) / (np.sum((h2_list - h2_mean) ** 2) + 1.0e-8)
    ...
    kap1 = (
        spp
        * np.sum((var_list - var_mean) * (hk_inv_list - hk_inv_mean))
...
def esim_simple(
    gray_values: NDArray[np.double], times: NDArray[np.double], pixel: Tuple[float, float], threshold: float = 0.2
) -> List[Any]:
    px, py = pixel
...
def esim_ours(
    log_gray_imgs: NDArray[np.double],
    gbuf_imgs: NDArray[np.double],
    ...
    n_wlr = 0
    thres2 = threshold**2
    prev_log_alph, prev_beta, prev_res, prev_Vp, prev_bb = wlr_method(
        gbuf_samples[0], log_gray_samples[0], i_ctr, x_var=var_gbuf_samples[0], Y_var=var_log_gray_samples[0], spp=spp
    )
    ...
    for i, t in enumerate(times):
        ...
        if abs(res - prev_res) > thres2:
            n_wlr += 1
            log_alph, beta, res, Vp, bb = wlr_method(
                gbuf_samples[i],
                log_gray_samples[i],
                i_ctr,
