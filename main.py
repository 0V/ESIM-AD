...
from esim import lin_log, esim_ours, esim_simple, var_lin_log, wlr_denoise
from utils import evs_save, gamma_tmo, video_save, tqdm_joblib, evs_to_video, extract_gbuffers


def main(args):
    # Make output folder
    out_dir = os.path.join(args.output, os.path.basename(args.data_root) + '_{:d}spp'.format(args.spp))
    ...
    for exr_file in tqdm(exr_files):
        color_img, albedo_img, normal_img, position_img, \
            var_color_img, var_albedo_img, var_normal_img, var_position_img \
    ...
    gray_imgs = color_imgs[..., 0] * 0.299 +\
                color_imgs[..., 1] * 0.587 +\
                color_imgs[..., 2] * 0.114
    var_gray_imgs = var_color_imgs[..., 0] * (0.299**2) +\
    ...
    with tqdm_joblib(img_width * img_height, desc="WLR denoise"):
        alph_imgs = joblib.Parallel(n_jobs=args.n_jobs)(joblib.delayed(wlr_denoise)(
            gray_imgs,
            gbuf_imgs,
            var_gray_imgs,
            var_gbuf_imgs,
    ...
    with tqdm_joblib(img_width * img_height, desc="ESIM-AD"):
        res_ours = joblib.Parallel(n_jobs=args.n_jobs)(joblib.delayed(esim_ours)(
            log_gray_imgs,
            gbuf_imgs,
            var_log_gray_imgs,
            var_gbuf_imgs,
    ...
    print("ours: %d events detected. %d WLR fits (%.2f %%)." % (len(ours_evs), wlr_count, 100.0 * wlr_count /
                                                                (img_width * img_height * num_frames)))
